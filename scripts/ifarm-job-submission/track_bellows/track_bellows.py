...
jsubf.write("#SBATCH --output="+tmp+"/%A_%a.out\n")
jsubf.write("#SBATCH --error="+tmp+"/%A_%a.err\n")
...
jsubf.write("cp "+args.home+"/* "+args.work_dir+"/${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}\n")
jsubf.write("cp -r "+args.src_dir+"/${FILE[${SLURM_ARRAY_TASK_ID}-1]} "+args.work_dir+"/${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}   \n")
jsubf.write("echo \"Current working directory is `pwd`\"\n")




jsubf.write("./reroot -q -b copytree_selectbr.C\"(\\\"${FILE[${SLURM_ARRAY_TASK_ID}-1]}/remollout.root\\\",\\\"analysed_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.root\\\")\"\n")
jsubf.write("mv analysed_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.root "+out+"\n")



jsubf.write("cd "+args.work_dir+"\n")
#jsubf.write("rm -rf ${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}\n")

jsubf.write("echo \"Program remoll finished with exit code $? at: `date`\"\n")
jsubf.close()
	        
                
subprocess.call("sbatch --array="+args.run_range+" "+jsub+"/job.sh",shell=True)
