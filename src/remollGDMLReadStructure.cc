...
remollGDMLReadStructure::remollGDMLReadStructure():G4GDMLReadStructure()
{
  auto rotatex=[](G4RotationMatrix& rot,G4ThreeVector angle) { rot.rotateX(angle.x()); };
  auto rotatey=[](G4RotationMatrix& rot,G4ThreeVector angle) { rot.rotateY(angle.y()); };
  auto rotatez=[](G4RotationMatrix& rot,G4ThreeVector angle) { rot.rotateZ(angle.z()); };
  rotations  = {
...
void remollGDMLReadStructure::RotationRead(const xercesc::DOMElement* const vectorElement, G4RotationMatrix& rot)
{
  G4double unit = 1.0;
  G4ThreeVector vec;
  ...
  for(auto rotate : rotations.at(order)) rotate(rot,vec);

}

G4LogicalVolume* remollGDMLReadStructure::FileRead(const xercesc::DOMElement* const fileElement)
{
  G4String name;
  G4String volname;
...
void remollGDMLReadStructure::PhysvolRead( const xercesc::DOMElement* const physvolElement, G4AssemblyVolume* pAssembly)
{
  G4String name;
...
void remollGDMLReadStructure::ParametersRead(const xercesc::DOMElement* const element)
{
  G4ThreeVector rotation(0.0, 0.0, 0.0);
  G4ThreeVector position(0.0, 0.0, 0.0);
  bool orotation_requested = false;
  ...
  for(xercesc::DOMNode* iter = element->getFirstChild(); iter != nullptr; iter = iter->getNextSibling())
  {
    ...
    {
      RotationRead(child,orotm);
      orotation_requested = true; // makes orotation get priority over normal rotation.
    }
    else if(tag == "position")
    {
      VectorRead(child, position);
...
void remollGDMLReadStructure::Volume_contentRead( const xercesc::DOMElement* const volumeElement)
{
