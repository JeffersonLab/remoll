...
double LabFrame(double pL[], double pR[], double polvL[],double mass[], double Asym[]);
...
if(Ngamma>100)Ngamma=100; // Photons[100] is set in hyperon.h
mp = mproton;
mkplus = mkaon;
s11 = (mp+k0)*(mp+k0)-k0*k0;// s for k0=11 GeV
x = s11*s11+(mkplus*mkplus-mhyp*mhyp)*(mkplus*mkplus-mhyp*mhyp);
x = (x -2.*(mkplus*mkplus + mhyp*mhyp)*s11)/4./s11;
pcmax11 = sqrt(x);//maximum cm momentum for k0=11 GeV
pKmax = gamma11*(pcmax11+beta11*sqrt(mkplus*mkplus+pcmax11*pcmax11));
...
ofile<<"pKmax = "<<pKmax<<" GeV/c"<<bl<<"pTmax = "<<bl<<pcmax11<<bl<<"mass_hyperon = "<<mhyp<<eol;
ofile<<"(x,y,z) in cm,     (px,py,pz) in GeV/c,      hyperons per electron"<<eol;
...
pKmag = pKmin;
if(pKmag<pKtrans)pKmag = pKtrans; // pKmag can't be smaller than pKtrans
...
weight =dTprotons*microbarn*twopi*dPt*dPk*pKtrans/(pK[0]*cos(theta))*dEQsiginv;
...
if(sumgamma>var)break;
kgamma=kgamma+dgamma;
}

kgamma = kgamma-dgamma + (var - (sumgamma-Photons[j]))*dgamma/Photons[j];
...
if((wt!="nan")&&(pos0!="nan")&&(pos1!="nan")&&(pos2!="nan")&&(pL1!="nan")&&(pL2!="nan")&&(pL3!="nan")){
ofile<<pos[0]<<bl<<pos[1]<<bl<<pos[2]<<bl<<pL[1]<<bl<<pL[2]<<bl<<pL[3]<<bl<<weight<<eol;
gdweight = gdweight +1;
}
pKmag = pKmag + dPk;
}
...
k=i/Ncnt;
if(k*Ncnt == i)std::cout<<"vertex number "<<i<<eol;
...
//************
double Vertex(long int j, double vertex[])
{
double zlo;
...
double Displacement(double pL[], double ctau)
{
...
double pLab,pT, ML, xR,E,pLcm,pcm,pLabL,kmin=0.,theta,ekplus,abst;
int nk=50, i;
...
ekplus = pK[0];
theta = asin(pT/pLab);
kmin=(mhyp*mhyp-mkplus*mkplus-mp*mp+2.*mp*ekplus)/2./(mp - ekplus + pLab*cos(theta));
Kmin = kmin;
t1 = a1 + a2/sqrt(s11);
t2a = 1. - xR + a3*a3/s11;
t2 = pow(t2a,a4);
...
//normalize Photons[i]
for(i=0;i<nk-1;i++)Photons[i]=Photons[i]/PhotonSum;
...
double HyperonMomentum(double pL[],double pK[], double kgamma, double pKz)
{
...
mx2 = (e0-ek)*(e0-ek)-kgamma*kgamma-pKmag*pKmag+2.*kgamma*pKz;
sx = mx2;
...
pcmx = sqrt(((mhyp*mhyp+mQ*mQ-sx)*(mhyp*mhyp+mQ*mQ-sx)-4.*mhyp*mhyp*mQ*mQ)/4./sx);
phiX = twopi*randf();
csthetaX = 2.*randf() - 1.;
...
pLX[3] = pcmx*csthetaX;
pLX[0] = sqrt(mhyp*mhyp+pLX[1]*pLX[1]+pLX[2]*pLX[2]+pLX[3]*pLX[3]);
...
for(j=0;j<4;j++)pL[0] = pL[0] + LT[0][j]*pLX[j];
for(j=0;j<4;j++)pL[1] = pL[1] + LT[1][j]*pLX[j];
for(j=0;j<4;j++)pL[2] = pL[2] + LT[2][j]*pLX[j];
for(j=0;j<4;j++)pL[3] = pL[3] + LT[3][j]*pLX[j];

if(mhyp == 1.192642){
for(i=0;i<4;i++)pL0[i]=pL[i]; // store sigma0 lab momentum
...
ksig0[0] = sqrt(mlambda*mlambda+ksig0[1]*ksig0[1]+ksig0[2]*ksig0[2]+ksig0[3]*ksig0[3]);
//Calculate the Lorentz transformation from X -> Lab frame
sign = -1; // sign = -1 for sigma0->Lab frame
var = Lorentz(pL0,LT,sign);
for(i=0;i<4;i++)pL[i]=0.;
for(j=0;j<4;j++)pL[0] = pL[0] + LT[0][j]*ksig0[j];
for(j=0;j<4;j++)pL[1] = pL[1] + LT[1][j]*ksig0[j];
for(j=0;j<4;j++)pL[2] = pL[2] + LT[2][j]*ksig0[j];
for(j=0;j<4;j++)pL[3] = pL[3] + LT[3][j]*ksig0[j];
}
return mx;
}
...
double Lorentz(double pL[],double LT[][4],int sign)
{
