        ...
        for (size_t j = 0; j < particle->size(); j++)
        {
            ...
            for (size_t k = 0; k < z->size(); k++)
            {
                if (z->at(k) == thisZ)
                {
                    ...
                    rates.push_back(tmp); // Each entry has a single rate
                    break;
                }
            }
        }
...
int main(){
    ...
    while (ans=='y')
    {
        ...
        if (ans=='y')
        {
            ...
            if (goodParticles >= 1)
            {
                TGraph2D *particleGraph = new TGraph2D(goodParticles,&particlesX[0],&particlesY[0],&rates[0]);
