...
class beamLineDetHistos {
 ...
 public:
  ...
  void initHisto_det5614(TFile*,const int,const string,const string,const float,const int);
  ...
  void fillHisto(const int detID, const int sp, const double rdDmg[3], const double pz,
		 ...
		 const float vx, const float vy, const float vz, 
		 const int subDet);
  void fillHisto_det5614(const int detID, const double rdDmg[3], const double pz,
			 const float xx, const float yy, const float kinE, const float rr,
			 const float vx, const float vy, const float vz,
			 const int subDet);
  ...
  void writeOutput_det5614(TFile *fout, int detID, double scaleFactor,const int subDet);
...
void beamLineDetHistos::fillHisto_det5614(const int detID, const double rdDmg[3], const double pz,
					  const float xx, const float yy, const float kinE, const float rr,
					  const float vx, const float vy, const float vz,
					  const int subDet=0) {
  if (ID2entry.find(detID+10000*subDet) == ID2entry.end())
...
void beamLineDetHistos::fillHisto_detX(const int detID, const double rdDmg[3], const double pz,
  ...
  for(int kk=0;kk<nDmg-1;kk++){
    if (detID==28) {    if(rr>600 && rr<1200) dXY[kk][0][det]->Fill(xx,yy,rdDmg[kk]);  }
    else dXY[kk][0][det]->Fill(xx,yy,rdDmg[kk]);
    if(pz<0){
      if (detID==28) {if(rr>600 && rr<1200) dXY[kk][2][det]->Fill(xx,yy,rdDmg[kk]);  }
      else dXY[kk][2][det]->Fill(xx,yy,rdDmg[kk]);
    }
    else{
      if (detID==28) {if(rr>600 && rr<1200) dXY[kk][1][det]->Fill(xx,yy,rdDmg[kk]);  }
      else dXY[kk][1][det]->Fill(xx,yy,rdDmg[kk]);
...
void beamLineDetHistos::initHisto_det5614(TFile *fout, const int detID, const string detNm, const string postfix = "",
                                          const float range=2000, const int subDet=0){
  fout->cd();
  fout->cd(Form("det%d",detID));
  ID2entry.insert(std::pair<int, int>(detID+10000*subDet,ID2entry.size()));
