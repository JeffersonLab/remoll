...
while [[ ${jobid} -le ${lastjob} ]] ; do
	...
	mkdir -p jobs/${batch}/${name}/
	mkdir -p macros/jobs/${name}/
	...
	sed -e "s|%nevents%|${nevents}|g" \
			...
			-e "s|%name%|${name}|g" \
		macros/${name}.in \
	> macros/jobs/${name}/${name}_${jobid}.mac
	...
	sed -e "s|%nevents%|${nevents}|g" \
			...
			-e "s|%name%|${name}|g" \
		jobs/job.${batch}.in \
	> jobs/${batch}/${name}/${name}_${jobid}.${batch}
  ...
  if [ -e ${rootfile} ] ; then
    ...
    if [ "${batch}" == "xml" ] ; then
      jsub -xml jobs/${batch}/${name}/${name}_${jobid}.${batch}
    fi
    if [ "${batch}" == "sh" ] ; then
      echo "Submitting job jobs/${batch}/${name}_${jobid}.${batch}..."
      qsub jobs/${batch}/${name}/${name}_${jobid}.${batch}
    fi
  fi
