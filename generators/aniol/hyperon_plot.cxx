...
TH1F *Rad = new TH1F("Rad","Distribution of Hyperons radially", 55,-5,50);
...
TH2F *h2 = new TH2F("h2","Zpos vs Pmag for Hyperons",350,-150,200, 120,0,12);
TH2F *h3 = new TH2F("h3","Radial_pos vs Pmag for Hyperons",55,-5,50, 40,0,4);
TH2F *h4 = new TH2F("h4","radial position vs Z for Hyperons",55,-5,50, 350,-150,200);
...
if(Nevents==1)std::cout<<x<<bl<<y<<bl<<z<<bl<<px<<bl<<py<<bl<<pz<<bl<<weight<<eol;// did we read the correct line?
notnan=notnan+1;
}
...
TCanvas *c2 = new TCanvas("c2","Hyperons Rad position",40,40,1000,1000);
c2->Divide(1);
c2->cd(1);
Rad->Draw();
...
TCanvas *c8 = new TCanvas("c8","Zpos vs Pmag of Hyperons",40,40,1000,1000);
c8->Divide(1);
c8->cd(1);
h2->Draw();

TCanvas *c9 = new TCanvas("c9","Rpos vs Pmag of Hyperons",40,40,1000,1000);
c9->Divide(1);
c9->cd(1);
h3->Draw();

TCanvas *c10 = new TCanvas("c10","Zpos vs Pmag of Hyperons",40,40,1000,1000);
c10->Divide(1);
c10->cd(1);
h2->Draw("SPEC dm(1,10) pa(2,1,1) ci(1,4,8) a(15,65,180)");

TCanvas *c11 = new TCanvas("c11","Rpos vs Pmag of Hyperons",40,40,1000,1000);
c11->Divide(1);
c11->cd(1);
h3->Draw("SPEC dm(1,10) pa(2,1,1) ci(1,4,8) a(15,65,180)");

TCanvas *c12 = new TCanvas("c12","radial posotion vs Z of Hyperons",40,40,1000,1000);
c12->Divide(1);
c12->cd(1);
h4->Draw();

TCanvas *c13 = new TCanvas("c13","radial position vs Z of Hyperons",40,40,1000,1000);
c13->Divide(1);
c13->cd(1);
