...
do
    ...
    cp preserve_recreate-brad.mac recreate-brad_${geom}_${name}.mac
    ...
    sed -i 's;'"/remoll/evgen/beam/th -11.5 deg"';'"/remoll/evgen/beam/th ${angle} deg"';g' recreate-brad_${geom}_${name}.mac
    sed -i 's;'"/remoll/evgen/beam/origin 0.0 0.0 0.0 mm"';'"/remoll/evgen/beam/origin ${x_pos} 0.0 ${z_point} cm"';g' recreate-brad_${geom}_${name}.mac

    sed -i 's;'"/remoll/setgeofile geometry_Mainz/mollerMother_Mainz.gdml"';'"/remoll/setgeofile mollerMother_${geom}.gdml"';g' recreate-brad_${geom}_${name}.mac

    sed -i 's;'"/remoll/evgen/beam/rasterRefZ 0.0 mm"';'"/remoll/evgen/beam/rasterRefZ ${z_point} cm"';g' recreate-brad_${geom}_${name}.mac

    sed -i 's;'"/process/optical/processActivation Cerenkov false"';'"/process/optical/processActivation Cerenkov ${cer}"';g' recreate-brad_${geom}_${name}.mac
    sed -i 's;'"/process/optical/processActivation Scintillation false"';'"/process/optical/processActivation Scintillation ${scint}"';g' recreate-brad_${geom}_${name}.mac
    sed -i 's;'"Mainz_0.0_degrees_0.0_x.root"';'"${geom}_${name}.root"';g' recreate-brad_${geom}_${name}.mac
    tmpFolder="out_${geom}_${name}"
    if [ ! -d $tmpFolder ] ; then
        mkdir $tmpFolder
    fi
    cd $tmpFolder
    ...
    cp ../pe .
    mv ../recreate-brad_${geom}_${name}.mac . 
    #source /share/apps/root-5.34.36-build/bin/thisroot.sh
    ./remoll recreate-brad_${geom}_${name}.mac
    #cd ../
    #./build/remoll macros/recreate-brad_${geom}_${name}.mac
    #cd -
    #source /share/apps/root-6.14.06-build/bin/thisroot.sh
    #cd $tmpFolder
    ./pe remollout_${geom}_${name}.root 540210 ${angle} ${x_pos} ${reflectivity} ${cerenkov} ${scintillation} ${z_point}
    ...
    rm remollout_${geom}_${name}_PEs_det_540210.root
    cd -
done
