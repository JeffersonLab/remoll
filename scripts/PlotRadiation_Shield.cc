...
Bool_t earlybreak=kTRUE;//kFALSE;//kTRUE;//kFALSE;//exit early from the main tree entries loop

Bool_t kSaveRootFile=kFALSE;//kTRUE;//kTRUE;//save histograms and canvases into a rootfile
Bool_t kShowGraphic=kTRUE;//kTRUE;//Show canvases and they will be saved the rootfile. Set this to false to show no graphics but save them to rootfile
...
//
  ...
  Tmol->Add("/home/cameronc/gitdir/dose_remoll/output/check3_500000*.root");



  TString rootfilename="/home/cameronc/gitdir/dose_remoll/output/check3_plots.root";//name of the rootfile to save generated histograms
...
//generic hit (for sens detectors)
  ...
  TString spid[3]={"abs(electrons)","#gamma","Neutron"};
  ...
  if (kShow1DKEPlots){
	...
	if (kSave1DKEHisto)
	  canvas_ke[i][j]->SaveAs(Form("/home/rakithab/Simulation_Analysis/Remoll/MOLLER_12GeV/Plots/hall_radiation/Test_tgt_shield/lead_shield_thickPoly_shieldblocks/moller_sim_ShlBlocks_canvas_ke_vtx%d_pid%d.png",i+1,j+1));
	if (kSaveRootFile)
	  canvas_ke[i][j]->Write();
      }
    }
  ...
  if (kShow1DVrtxPlots){
	...
	if (kSave1DVrtxHisto){
	  canvas_vertx[i][j]->SaveAs(Form("/home/rakithab/Simulation_Analysis/Remoll/MOLLER_12GeV/Plots/hall_radiation/Test_tgt_shield/lead_shield_thickPoly_shieldblocks/moller_sim_ShlBlocks_canvas_vrtx_vrtx%d_pid%d.png",i+1,j+1));
	  canvas_vertx_noWeight[i][j]->SaveAs(Form("/home/rakithab/Simulation_Analysis/Remoll/MOLLER_12GeV/Plots/hall_radiation/Test_tgt_shield/lead_shield_thickPoly_shieldblocks/moller_sim_ShlBlocks_canvas_vrtx_noWeight_vrtx%d_pid%d.png",i+1,j+1));
	  canvas_vertx_integral[i][j]->SaveAs(Form("/home/rakithab/Simulation_Analysis/Remoll/MOLLER_12GeV/Plots/hall_radiation/Test_tgt_shield/lead_shield_thickPoly_shieldblocks/moller_sim_ShlBlocks_canvas_vrtx_integral_vrtx%d_pid%d.png",i+1,j+1));
	}
	if (kSaveRootFile){
  ...
  if (kShow2Dhits){
	...
	for(Int_t k=0;k<3;k++){//detid
      ...
      if (kSave2DHisto)
	canvas_hallrad[i]->SaveAs(Form("/home/rakithab/Simulation_Analysis/Remoll/MOLLER_12GeV/Plots/hall_radiation/Test_tgt_shield/lead_shield_thickPoly_shieldblocks/moller_sim_ShlBlocks_canvas_hallrad_vrtx%d.png",i+1));
      if (kSaveRootFile)
	...
	for(Int_t k=0;k<2;k++){//detid
      ...
      if (kSave2DHisto)
	canvas_hallrad_cyc_xy[i]->SaveAs(Form("/home/rakithab/Simulation_Analysis/Remoll/MOLLER_12GeV/Plots/hall_radiation/Test_tgt_shield/lead_shield_thickPoly_shieldblocks/moller_sim_ShlBlocks_canvas_hallrad_cyc_xy_vrtx%d.png",i+1));
      if (kSaveRootFile)
  ...
  if (kShow2DVertex){
	...
	for(Int_t k=0;k<3;k++){//detid
	  ...
	  HistoVertex_RadDet[i][j][k][0]->Draw("colz");	
	...
	canvas_hallrad_xy_vrtx[i]->SaveAs(Form("/home/rakithab/Simulation_Analysis/Remoll/MOLLER_12GeV/Plots/hall_radiation/Test_tgt_shield/lead_shield_thickPoly_shieldblocks/moller_sim_ShlBlocks_canvas_hallrad_xy_vrtx%d.png",i+1));
	canvas_hallrad_rz_vrtx[i]->SaveAs(Form("/home/rakithab/Simulation_Analysis/Remoll/MOLLER_12GeV/Plots/hall_radiation/Test_tgt_shield/lead_shield_thickPoly_shieldblocks/moller_sim_ShlBlocks_canvas_hallrad_rz_vrtx%d.png",i+1));
      }
      if (kSaveRootFile){
  ...
  strline="/home/rakithab/Simulation_Analysis/Remoll/MOLLER_12GeV/Rootfiles/";//main root files used//_allshield_BorConcrete
  list_power->Add(new TObjString(strline));
