...
Bool_t kVertices=kFALSE;              // Governs the cut region plotting
Bool_t kShlds=kFALSE;                 // Governs the sensitive shielding region vertex position plotting
Bool_t kShldHits=kTRUE;               // Governs the sensitive shielding region hit position plotting

Bool_t kSave1DKEHisto=kVertices;      // option to save Kinetic energy from cut regions histograms
Bool_t kShow1DKEPlots=kVertices;      
Bool_t kSave1DShldKEHisto=kShlds;     // option to save Kinetic energy going into the different shielding block destination regions
Bool_t kShow1DShldKEPlots=kShlds;     

Bool_t kSave1DVrtxHisto=kVertices;    // option to save the z vertex positions from the cut regions histograms
Bool_t kShow1DVrtxPlots=kVertices;    
Bool_t kSave1DShldVrtxHisto=kShlds;   // option to save the z vertex positions going into the different shielding block destination regions
Bool_t kShow1DShldVrtxPlots=kShlds;
Bool_t kSave1DShldHitHisto=kShldHits; // option to save the z hit positions going into the different shielding block destination regions 
Bool_t kShow1DShldHitPlots=kShldHits;
...
int main(Int_t argc,Char_t* argv[]) {
  ...
  TString added_file="/home/cameronc/gitdir/dose_remoll/output/beam_tracking_10M_NEW.root";
  TString plotsFolder="/home/cameronc/gitdir/dose_remoll/output/Plots_tracking_10M_NEW/";//Name of folder for saving plots
  TString rootfilename=plotsFolder+"tracking_10M_plots_NEW.root";//name of the rootfile to save generated histograms
  list_outputs.open(plotsFolder+"list_outputs_tracking_10M_NEW.txt");
  ...
  //
  TH2F *Histo_RadDet[n_regions][n_particles][5];//n_regions vertex ranges for three particles species for three detectors (0-cylinder phi-y, 1 and 2 are top and bottom disk detectors, 3,4 is cylinder x-y forward backward)

  TH2F *HistoVertex_RadDet[n_regions][n_particles][n_energy_ranges][2];//n_regions vertex ranges for three particles species for three energy ranges for y:x, and y:z NEW //I may extend to add x:z, and y:z vertex distributions
  TH2F *HistoVertex_shld_RadDet[n_shlds][n_particles][n_energy_ranges][2];//n_shlds vertex ranges for three particles species for three energy ranges for y:x, and y:z NEW //I may extend to add x:z, and y:z vertex distributions
  TH2F *HistoHit_RadDet[n_regions][n_particles][n_energy_ranges][2];//n_regions vertex ranges for three particles species for three energy ranges for y:x, and y:z NEW //I may extend to add x:z, and y:z vertex distributions
  TH2F *HistoHit_shld_RadDet[n_shlds][n_particles][n_energy_ranges][2];//n_shlds vertex ranges for three particles species for three energy ranges for y:x, and y:z NEW //I may extend to add x:z, and y:z vertex distributions
  ...
  for(Int_t i=0;i<n_shlds;i++){
    for(Int_t j=0;j<n_particles;j++){
      for(Int_t k=0;k<n_energy_ranges;k++){
        ...
        Histo_shld_hit[i][j][k]=new TH1F(Form("Histo_shld_hit_%d_%d_%d",i+1,j+1,k+1),Form("%s Hits into %s Area in %s MeV Range (KE weighted);Z vertex (cm);W/#muA",spid[j].Data(),svertex_shld[i].Data(),ke_range[k].Data()),z_vertex_bin_counts[0],Hall_z_vertices_low-1,Hall_z_vertices_up+1);
	      Histo_shld_hit_noWeight[i][j][k]=new TH1F(Form("Histo_shld_hit_noWeight_%d_%d_%d",i+1,j+1,k+1),Form("%s Hits into %s Area in %s MeV Range ;Z vertex (cm);Hz/#muA",spid[j].Data(),svertex_shld[i].Data(),ke_range[k].Data()),z_vertex_bin_counts[0],Hall_z_vertices_low-1,Hall_z_vertices_up+1);
        HistoHit_shld_RadDet[i][j][k][0]=new TH2F(Form("HistoHit_shld_RadDet_v%d_p%d_k%d_0",i+1,j+1,k+1),Form(" %s Hits into %s Area in %s MeV Range; x (cm); y (cm); (W/#muA)",spid[j].Data(),svertex_shld[i].Data(),ke_range[k].Data()),x_vertex_bin_counts[0],Hall_x_vertices_low-1,Hall_x_vertices_up+1,y_vertex_bin_counts[0],Hall_y_vertices_low-1,Hall_y_vertices_up+1);
	      HistoHit_shld_RadDet[i][j][k][1]=new TH2F(Form("HistoHitx_shld_RadDet_v%d_p%d_k%d_2",i+1,j+1,k+1),Form(" %s Hits into %s Area in %s MeV Range; z (cm); y (cm); (W/#muA)",spid[j].Data(),svertex_shld[i].Data(),ke_range[k].Data()),z_vertex_bin_counts[0],Hall_z_vertices_low-1,Hall_z_vertices_up+1,y_vertex_bin_counts[0],Hall_y_vertices_low-1,Hall_y_vertices_up+1);
      }
    }
  }
...
//        } // Old end of for-loop, not it will add the event to any region's histgrams that may or may not overlap (before it was exclusive to just the first region that got it -> removed the 'breaks'). CSC 7/26/2017 EDIT
     ...
     /*   if (keid>=0 && hit_radius > hit_radius_min[vrtx_z]){
      ...
      if((kShlds || kShldHits) && (fGenDetHit_det[j]==SensVolume_v[3] || fGenDetHit_det[j]==SensVolume_v[4] || fGenDetHit_det[j]==SensVolume_v[5] || fGenDetHit_det[j]==SensVolume_v[6] || fGenDetHit_det[j]==SensVolume_v[7] || fGenDetHit_det[j]==SensVolume_v[8] || fGenDetHit_det[j]==SensVolume_v[9] || fGenDetHit_det[j]==SensVolume_v[10] || fGenDetHit_det[j]==SensVolume_v[11] || fGenDetHit_det[j]==SensVolume_v[12] || fGenDetHit_det[j]==SensVolume_v[13] || fGenDetHit_det[j]==SensVolume_v[14] || fGenDetHit_det[j]==SensVolume_v[15]) && (TMath::Abs(fGenDetHit_pid[j])==11 || fGenDetHit_pid[j]==22 || fGenDetHit_pid[j]==2112) ){//total into the hall
	      //big set of for loops!!
	      detid=detectormap[fGenDetHit_det[j]]; 
        pid=pidmap[(Int_t)TMath::Abs(fGenDetHit_pid[j])];
        ...
        if(vrtx>=0){
          ...
          if (keid>=0){
            ...
            Histo_shld_hit_noWeight[vrtx][pid][keid]->Fill(fGenDetHit_Z[j]*100,6.241e+12/n_events);
...
// \NEW
  ...
  //2D radiation distr on cylinder and disk detectors
    Double_t hallrad_color_max=0;
    Double_t hallrad_color_min=0;
    for(Int_t i=0;i<n_regions;i++){//region
      for(Int_t j=0;j<n_particles;j++){//pid
      	for(Int_t k=1;k<n_energy_ranges;k++){//detid (start at 1 to skip counting the phi plot)
          if (Histo_RadDet[i][j][k]->GetMinimumStored()<hallrad_color_min) hallrad_color_min = Histo_RadDet[i][j][k]->GetMinimumStored();	
          if (Histo_RadDet[i][j][k]->GetMaximumStored()>hallrad_color_max) hallrad_color_max = Histo_RadDet[i][j][k]->GetMaximumStored();	
        }
      }
    }
    ...
    for(Int_t i=0;i<n_regions;i++){//vertex // THESE ARE THE TOP Heatmaps vs. vertex region
      ...
      for(Int_t j=0;j<n_particles;j++){//pid
          ...
          Histo_RadDet[i][j][k]->SetMinimum(hallrad_color_min);	
          Histo_RadDet[i][j][k]->SetMaximum(hallrad_color_max);	
  ...
  if (kShow2DCylPlots){
    Double_t hallrad_cyc_color_max=0;
    Double_t hallrad_cyc_color_min=0;
    for(Int_t i=0;i<n_shlds;i++){//region
      for(Int_t j=0;j<n_particles;j++){//pid
      	for(Int_t k=3;k<5;k++){//detid (just the front and back hallrad scatter plots)
          if (Histo_RadDet[i][j][k]->GetMinimumStored()<hallrad_cyc_color_min) hallrad_cyc_color_min = Histo_RadDet[i][j][k]->GetMinimumStored();
          if (Histo_RadDet[i][j][k]->GetMaximumStored()>hallrad_cyc_color_max) hallrad_cyc_color_max = Histo_RadDet[i][j][k]->GetMaximumStored();	
        }
      }
    }
    ...
    for(Int_t i=0;i<n_regions;i++){//vertex
      ...
      for(Int_t j=0;j<n_particles;j++){//pid
          ...
          Histo_RadDet[i][j][3+k]->SetMinimum(hallrad_cyc_color_min);	// start at 3 because we are skipping the phi, top, and side hallrad plots.
          Histo_RadDet[i][j][3+k]->SetMaximum(hallrad_cyc_color_max);	
  ...
  if (kShow2DVertexPlots){
    Double_t hallrad_vert_color_max[2]=0;
    Double_t hallrad_vert_color_min[2]=0;
    for(Int_t i=0;i<n_regions;i++){//region
      for(Int_t j=0;j<n_particles;j++){//pid
        for(Int_t l=0;l<n_energy_ranges;l++){//pid
      	  for(Int_t k=0;k<2;k++){//kinds of plots = 2 
            if (HistoVertex_RadDet[i][j][l][k]->GetMinimumStored()<hallrad_vert_color_min[k]) hallrad_vert_color_min[k] = HistoVertex_RadDet[i][j][l][k]->GetMinimumStored();
            if (HistoVertex_RadDet[i][j][l][k]->GetMaximumStored()>hallrad_vert_color_max[k]) hallrad_vert_color_max[k] = HistoVertex_RadDet[i][j][l][k]->GetMaximumStored();	
          }
        }
      }
    }
    ...
    for(Int_t i=0;i<n_regions;i++){//vertex
      ...
      for(Int_t j=0;j<n_particles;j++){//pid
        for(Int_t k=0;k<n_energy_ranges;k++){//energy_ranges           
          ...
          HistoVertex_RadDet[i][j][k][0]->SetMinimum(hallrad_vert_color_min[0]);
          HistoVertex_RadDet[i][j][k][0]->SetMaximum(hallrad_vert_color_max[0]);	
          ...
          HistoVertex_RadDet[i][j][k][1]->SetMinimum(hallrad_vert_color_min[1]);	
          HistoVertex_RadDet[i][j][k][1]->SetMaximum(hallrad_vert_color_max[1]);	
          HistoVertex_RadDet[i][j][k][1]->Draw("colz");	
          HistoVertex_RadDet[i][j][k][1]->SetStats(0);
        }
      }
  ...
  if (kShow2DShldVertexPlots){
    Double_t hallrad_shld_color_max[2]=0;
    Double_t hallrad_shld_color_min[2]=0;
    for(Int_t i=0;i<n_regions;i++){//region
      for(Int_t j=0;j<n_particles;j++){//pid
        for(Int_t l=0;l<n_energy_ranges;l++){//pid
      	  for(Int_t k=0;k<2;k++){//kinds of plots = 2 
            if (HistoVertex_shld_RadDet[i][j][l][k]->GetMinimumStored()<hallrad_shld_color_min[k]) hallrad_shld_color_min[k] = HistoVertex_shld_RadDet[i][j][l][k]->GetMinimumStored();
            if (HistoVertex_shld_RadDet[i][j][l][k]->GetMaximumStored()>hallrad_shld_color_max[k]) hallrad_shld_color_max[k] = HistoVertex_shld_RadDet[i][j][l][k]->GetMaximumStored();	
          }
        }
      }
    }
    ...
    for(Int_t i=0;i<n_shlds;i++){//vertex
      ...
      for(Int_t j=0;j<n_particles;j++){//pid
        for(Int_t k=0;k<n_energy_ranges;k++){//energy ranges    
          ...
          HistoVertex_shld_RadDet[i][j][k][0]->SetMinimum(hallrad_shld_color_min[0]);	
          HistoVertex_shld_RadDet[i][j][k][0]->SetMaximum(hallrad_shld_color_max[0]);	
          ...
          HistoVertex_shld_RadDet[i][j][k][1]->SetMinimum(hallrad_shld_color_min[1]);	
          HistoVertex_shld_RadDet[i][j][k][1]->SetMaximum(hallrad_shld_color_max[1]);	
  ...
  if (kShow2DShldHitPlots){
    Double_t hallrad_shld_hit_color_max[2]=0;
    Double_t hallrad_shld_hit_color_min[2]=0;
    for(Int_t i=0;i<n_regions;i++){//region
      for(Int_t j=0;j<n_particles;j++){//pid
        for(Int_t l=0;l<n_energy_ranges;l++){//pid
      	  for(Int_t k=0;k<2;k++){//kinds of plots = 2 
            if (HistoHit_shld_RadDet[i][j][l][k]->GetMinimumStored()<hallrad_shld_hit_color_min[k]) hallrad_shld_hit_color_min[k] = HistoHit_shld_RadDet[i][j][l][k]->GetMinimumStored();
            if (HistoHit_shld_RadDet[i][j][l][k]->GetMaximumStored()>hallrad_shld_hit_color_max[k]) hallrad_shld_hit_color_max[k] = HistoHit_shld_RadDet[i][j][l][k]->GetMaximumStored();	
          }
        }
      }
    }
    ...
    for(Int_t i=0;i<n_shlds;i++){//vertex
      ...
      for(Int_t j=0;j<n_particles;j++){//pid
        for(Int_t k=0;k<n_energy_ranges;k++){//energy ranges    
          canvas_shld_hit_hallrad_xy_vrtx[i]->cd(n_particles*j+1+k);
          canvas_shld_hit_hallrad_xy_vrtx[i]->cd(n_particles*j+1+k)->SetLogz();
          HistoHit_shld_RadDet[i][j][k][0]->SetMinimum(hallrad_shld_hit_color_min[0]);	
          HistoHit_shld_RadDet[i][j][k][0]->SetMaximum(hallrad_shld_hit_color_max[0]);	
          ...
          canvas_shld_hit_hallrad_yz_vrtx[i]->cd(n_particles*j+1+k);   
          canvas_shld_hit_hallrad_yz_vrtx[i]->cd(n_particles*j+1+k)->SetLogz();
          HistoHit_shld_RadDet[i][j][k][1]->SetMinimum(hallrad_shld_hit_color_min[1]);	
          HistoHit_shld_RadDet[i][j][k][1]->SetMaximum(hallrad_shld_hit_color_max[1]);	
  ...
  for(Int_t i=0;i<n_particles;i++){//pid
    for(Int_t j=0;j<n_energy_ranges;j++){//energy range
      ...
      for(Int_t k=0;k<2;k++){//detector                             // FIXME number of hall (roof, walls) detectors present
  ...
  for (Int_t i=0;i<n_regions;i++){
    for(Int_t j=0;j<n_particles;j++){//pid
      for(Int_t k=0;k<n_energy_ranges;k++){//energy range
	      ...
	      sprintf(line1," ");//empty previous values
	      for(Int_t l=0;l<2;l++){//detector                             // FIXME number of hall (roof, wall) detectors present
  ...
  for (Int_t i=0;i<n_regions;i++){
    for(Int_t j=0;j<n_particles;j++){//pid
      for(Int_t k=0;k<n_energy_ranges;k++){//energy range
        ...
        sprintf(line," %20s %30s %20s",svertex[i].Data(),chpid[j],chenrange[k]);
        sprintf(line1," ");//empty previous values
        for(Int_t l=0;l<2;l++){//detector                             // FIXME number of hall (roof, walls) detectors present
