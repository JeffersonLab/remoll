...
int main(int argc, char **argv){
  ...
  if(suffix!="")
    ...
    foutNm = Form("%s_hallRad.root",finNm.substr(0,finNm.find_last_of(".")).c_str());
  fout=new TFile(foutNm.c_str(),"RECREATE");
  cout<<"write to "<<foutNm<<"\n\t process: "<<finNm<<endl;
    ...
    for (size_t j = 0; j < fGenDetHit->size(); j++)
      {
        ...
        //logX(Energy)
        hTotal[nHist][nPart][0]->Fill(kinE);
        valAvg[nHist][nPart][0]->Fill(kinE);
        //linX(Energy)
        hTotal[nHist][nPart][4]->Fill(kinE);
        valAvg[nHist][nPart][4]->Fill(kinE);

        //logX(Energy)
        hTotal[nHist][nPart][3]->Fill(Edeposit);
        valAvg[nHist][nPart][3]->Fill(Edeposit);
        //linX(Energy)
        hTotal[nHist][nPart][6]->Fill(Edeposit);
        valAvg[nHist][nPart][6]->Fill(Edeposit);

        double val(-1);
        val = radDmg.getNEIL(pdgID,kinE,0);
        if(val!=-999){
          //logX(Energy)
          hTotal[nHist][nPart][1]->Fill(kinE,val);
          valAvg[nHist][nPart][1]->Fill(kinE,val);
          //linX(Energy)
          hTotal[nHist][nPart][5]->Fill(kinE,val);
          valAvg[nHist][nPart][5]->Fill(kinE,val);
        }

        val = radDmg.getMREM(pdgID,kinE,0);
        if(val!=-999){
          hTotal[nHist][nPart][2]->Fill(kinE,val);
          valAvg[nHist][nPart][2]->Fill(kinE,val);
        }
      }
    evNr++;
    delete fGenDetHit;
