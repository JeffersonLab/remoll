...
// >.L analysis/usShieldAna.C
// > usShieldAna(<remoll output file>)
...
//det 91, 92
TH2D *hZPhi[2], *hRateZPhi[2];

//det 5547, 5543
TH2D *tXY[2],*tRateXY[2];
...
void usShieldingAna(const string& finName = "./remollout.root"){
  fin = finName;
  initHisto();
  process();
  writeOutput();
...
void initHisto(){
  string foutNm = Form("%s_usShieldAna.root",fin.substr(0,fin.find(".")).c_str());
  ...
  fout->mkdir("det28","quality assurance plots");
  fout->cd("det28");
  ...
  fout->mkdir("det9X","cylinder tubes");
  fout->cd("det9X");
  float llim[2]={-1500,3100};
  float ulim[2]={3000,20000};  
  for(int i=0;i<2;i++){
    hZPhi[i]=new TH2D(Form("hZPhi_9%d",i+1),Form("number of hits for det 9%d;z [mm];#phi [deg]",i+1),
		      200,llim[i],ulim[i],180,-180,180);
    hRateZPhi[i]=new TH2D(Form("hRateZPhi_9%d",i+1),Form("rate weighted of hits for det 9%d;z [mm];#phi [deg]",i+1),
			  200,llim[i],ulim[i],180,-180,180);
  }

  fout->mkdir("det554X","target dets");
  fout->cd("de554X");
  string hnm[2]={"in","out"};
  float xlim[2]={4000,1500};
  float ylim[2]={4000,2000};
  for(int i=0;i<2;i++){
    tXY[i]=new TH2D(Form("tXY_%s",hnm[i].c_str()),Form("number of hits for det %s;x [mm];y [mm]",hnm[i].c_str()),
		    200,-xlim[i],xlim[i],200,-ylim[i],ylim[i]);
    
						 
    tRateXY[i]=new TH2D(Form("tRateXY_%s",hnm[i].c_str()),Form("rate wgt hits for det %s;x [mm];y [mm]",hnm[i].c_str()),
			200,-xlim[i],xlim[i],200,-ylim[i],ylim[i]);
...
long processOne(string fnm){
  ...
  for (Long64_t event = 0; event < nEntries; t->GetEntry(event++)) {
	...
	afterColl2P[hit->at(j).trid-1] = hit->at(j).p;
      ...
      if(hit->at(j).det>90 && hit->at(j).det<93){
	int dt=hit->at(j).det%10-1;
	double phi = 180/pi * atan2(hit->at(j).y,hit->at(j).x);
	hZPhi[dt]->Fill(hit->at(j).z, phi);
	hRateZPhi[dt]->Fill(hit->at(j).z, phi,rate);
      }

      if(hit->at(j).det==5543 || hit->at(j).det==5547){
	int dt = (hit->at(j).det>5545) ? 0 : 1;
	tXY[dt] -> Fill(hit->at(j).x, hit->at(j).y);
	tRateXY[dt] -> Fill(hit->at(j).x, hit->at(j).y,rate);
      }
...
int findDetector(int &sector, double phi, double r){
  ...
  const double rMin[nRings][nSec]={
    {   0.0,    0.0,    0.0},//minimum set to 0 so that we catch everything in the det plane
    { 680.0,  680.0,  680.0},
    { 730.0,  730.0,  730.0},
    { 805.0,  827.5,  835.0},
    { 855.0,  900.0,  915.0},
  ...
  const double rMax[nRings][nSec]={
    ...
    { 855.0,  900.0,  915.0},
    {1070.0, 1060.0, 1055.0},
    {1500.0, 1500.0, 1500.0} //max set to 1.5m so that we can catch everything in det plane
  };
