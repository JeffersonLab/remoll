...
double alpha=0.645,Sr=0.75,h=-1.,frac=0.36,mhyp=mLambda,ma=mpi0,mb=mneutron,sign=-1.;
...
std::string nan = ("nan");
char bl = ' ', eol = '\n';
int icnts=0, j, ncnts=1000;

std::ifstream hyperons("../hyperons/lambda_outp.dat");
std::ofstream ofile("lambda-pi.dat");
...
kmag=sqrt(( (ma*ma+mb*mb-mhyp*mhyp)*(ma*ma+mb*mb-mhyp*mhyp)-4.*ma*ma*mb*mb)/4./mhyp/mhyp);
std::cout<<"kmag = "<<kmag<<eol;
while(!hyperons.eof()){
hyperons>>xv>>yv>>zv>>pL[1]>>pL[2]>>pL[3]>>weight;
//setup hyperon Lab 4 momentum
pL[0]=sqrt(pL[1]*pL[1]+pL[2]*pL[2]+pL[3]*pL[3]+mhyp*mhyp);
...
r = randf();
con = alpha*Sr*h;
x = h*sqrt(2.*(2.*r + con/2. -1.)/con +1./con/con) -1./con;
sntheta = sqrt(1.-x*x);
ka[3] = kmag*x;
phi = twopi*randf();
...
for(j=0;j<4;j++)pL[j]=0.;// zero out the 4 momentum 
for(j=0;j<4;j++)pL[0] = pL[0] + LT[0][j]*ka[j];
for(j=0;j<4;j++)pL[1] = pL[1] + LT[1][j]*ka[j];
for(j=0;j<4;j++)pL[2] = pL[2] + LT[2][j]*ka[j];
for(j=0;j<4;j++)pL[3] = pL[3] + LT[3][j]*ka[j];

//generate photons from pi0 decay
if(ma==mpi0){
for(j=0;j<4;j++)pL0[j]=pL[j]; //store pi0 lab momentum
...
for(j=0;j<4;j++)pL[j] = 0.;
for(j=0;j<4;j++)pL[0] = pL[0] + LT[0][j]*kgam[j];
for(j=0;j<4;j++)pL[1] = pL[1] + LT[1][j]*kgam[j];
for(j=0;j<4;j++)pL[2] = pL[2] + LT[2][j]*kgam[j];
for(j=0;j<4;j++)pL[3] = pL[3] + LT[3][j]*kgam[j];
}

weight = weight*frac; //renormalize for this channel particle a
ofile<<xv<<bl<<yv<<bl<<zv<<bl<<pL[1]<<bl<<pL[2]<<bl<<pL[3]<<bl<<weight<<eol;
icnts = icnts+1;
if((icnts/ncnts)*ncnts==icnts)std::cout<<"icnts "<<icnts<<eol;
}
...
double Lorentz(double pL[],double LT[][4],double sign)
{
